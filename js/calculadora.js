...
(function () {
  ...
  function recalc() {
    ...
    // Decreto vigente por material (8)
    $('s-decreto').textContent = '⚖️ ' + (Store.DAMU.decretos[$('s-cat').value] || '');

    // Logística inversa rentable (1 + 2)
    $('o-ingreso').textContent = clp(r.ingreso);
    $('o-costo').textContent = clp(r.costoTotal);
    var neto = $('o-neto');
    neto.textContent = (r.neto >= 0 ? '+' : '−') + clp(Math.abs(r.neto)).slice(1);
    neto.className = r.neto >= 0 ? 'text-emerald-400' : 'text-red-400';
    $('o-roi').textContent = r.neto >= 0
      ? 'Operación rentable: el material recuperado cubre el costo (ROI ' + r.roi + '%).'
      : 'Operación con costo neto: ROI ' + r.roi + '% (el ingreso no cubre la gestión).';
  ...
  function generarPDF() {
    ...
    fila('Decreto / instrumento', (Store.DAMU.decretos[$('s-cat').value] || '').replace(/[—–]/g, '-'));
    fila('Ingreso por material recuperado', clp(ultimo.ingreso));
    fila('Resultado neto (logistica inversa)', (ultimo.neto >= 0 ? '+' : '-') + clp(Math.abs(ultimo.neto)).slice(1));
