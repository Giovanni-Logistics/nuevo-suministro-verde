...
(function () {
  ...
  var sala = null, alias = null, rutaActiva = null, chartT = null;
  ...
  function refrescarKPIs() {
    var m = Store.metricasTransportista(sala.id, alias);
    $('t-km').textContent     = m.kmRecorridos;
    $('t-entreg').textContent = m.entregadas;
    $('t-curso').textContent  = m.enCurso;
    $('t-co2').textContent    = m.co2eRutasKg;
    $('t-kg').textContent     = m.kgTransportados.toLocaleString('es-CL');
    $('t-diesel').textContent = '$' + m.costoDieselCLP.toLocaleString('es-CL');
    pintarChart(m.porCategoriaKg);
  }
  ...
  function pintarChart(porCat) {
    ...
    if (chartT) { chartT.data.datasets[0].data = data; chartT.update(); return; }
    chartT = new Chart($('t-chart'), {
      type: 'bar',
